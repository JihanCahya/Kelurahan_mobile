...
import com.polinema.uas.sipkburengan.databinding.ActivityEditKtpBinding

class EditSuratKtpActivity : AppCompatActivity() , View.OnClickListener {

    private lateinit var b: ActivityEditKtpBinding
    private lateinit var db: DatabaseReference
    private lateinit var storageReference: StorageReference
    private lateinit var suratId: String
    lateinit var adapterSpin: ArrayAdapter<String>
    val arrayJenisKtp = arrayOf("Ktp Baru", "Ktp Hilang")
    ...
    private lateinit var oldImageUrlAkta: String
    private lateinit var oldImageUrlKK: String
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        b = ActivityEditKtpBinding.inflate(layoutInflater)
        ...
        adapterSpin = ArrayAdapter(this, android.R.layout.simple_list_item_1, arrayJenisKtp)
        b.spEdKtp.adapter = adapterSpin
        b.spEdKtp.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
        ...
        b.btnSimpanEdKtp.setOnClickListener(this)
        b.edFcKkKtp.setOnClickListener(this)
        b.edPengantarKtp.setOnClickListener(this)
        b.edKiaKtp.setOnClickListener(this)
        b.edFcAktaKtp.setOnClickListener(this)
    ...
    private fun uploadAndUpdateImage(imageType: String, imageUri: Uri) {
        ...
        uploadImage(imageType, imageUri, timestamp) { imageUrl ->
            ...
            when (imageType) {
                ...
                "akta" -> imageUrlAkta = imageUrl
                "kk" -> imageUrlKK = imageUrl
            }
    ...
    private fun openFileChooserKia() {
        val intent = Intent(Intent.ACTION_PICK)
        intent.type = "image/*"
        startActivityForResult(intent, PICK_IMAGE_REQUEST_KIA)
    }
    ...
    private fun openFileChooserKK() {
        val intent = Intent(Intent.ACTION_PICK)
        intent.type = "image/*"
        startActivityForResult(intent, PICK_IMAGE_REQUEST_KK)
    }

    private fun openFileChooserAkta() {
        val intent = Intent(Intent.ACTION_PICK)
        intent.type = "image/*"
        startActivityForResult(intent, PICK_IMAGE_REQUEST_AKTA)
    }
                ...
                if (dataSnapshot.exists()) {
                    ...
                    history?.let {
                        with(b) {
                            ...
                            oldImageUrlAkta = it.imageUrlAkta  // Store the old KTP image URL
                            oldImageUrlKK = it.imageUrlKK  // Store the old Pengantar RT image URL
                            status = it.status
                            ...
                            Glide.with(this@EditSuratKtpActivity)
                                .load(it.imageUrlAkta)
                                .into(imEdAktaKtp)

                            val position = arrayJenisKtp.indexOf(it.jenisSurat)
                            spEdKtp.setSelection(position)
                        }
                    }
                }
            }
    ...
    private fun createHistoryObject(): History {
        return History(
            ...
            adapterSpin.getItem(b.spEdKtp.selectedItemPosition)!!, status,
    ...
    companion object {
        private const val PICK_IMAGE_REQUEST_KIA = 1
        private const val PICK_IMAGE_REQUEST_PENGANTAR_RT = 2
        private const val PICK_IMAGE_REQUEST_AKTA = 3
        private const val PICK_IMAGE_REQUEST_KK = 4
    }
}